export function findTypeDClosest(
...
export function findFirstOrLastTypeD(checkpoints: checkpoint[], findFirst: boolean): checkpoint | null {
    if (findFirst) {
...
export function genGoogleMapsURL(checkpoints: (checkpoint | null)[]): string {
    ...
    const formattedLocations = checkpoints.map(checkpoint =>
        checkpoint && checkpoint.locationName
            ? encodeURIComponent(checkpoint.locationName.trim())
            : ''
    );
