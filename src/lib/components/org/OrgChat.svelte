...
<ChatComponent tripID={message.IDTrip} custID={message.IDAccount} bind:showChatPopup={showCustPopup} hasToken={true}/>
    ...
    <div class="border-2 rounded-lg p-4 mb-4 space-y-2 {(message.readed === false) ? 'border-accent1' : 'border-gray-300'}">
        ...
        <button class="w-full" on:click={()=>{showCustPopup = true}}>
    ...
    <button on:click={()=>{showCustPopup = true}}>
        ...
        <div class="flex gap-4 justify-center">
            {#if message.bookDone !== ""}
            <button class="grow	" on:click={()=>{showCustPopup = true}}>
