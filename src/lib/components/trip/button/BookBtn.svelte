...
$: if (showBookPopup) {
    fetchStatus().then(fetchedMessages => {
...
<Popup bind:isOpen={showBookPopup}>
    ...
    <div class="flex flex-col items-center">
        ...
        <div class="flex gap-8 my-10 text-lg">
            {#if currentStatus == "BI"}
            <button on:click={()=>{confirmPoup = true;cancel = true}}>
                <ButtonMine background={"bg-error"}>
                    ยกเลิก
                </ButtonMine>
            </button>
            {/if}
            {#if currentStatus == "NM"}
            <button on:click={()=>{confirmPoup = true;book = true}}>
                <ButtonMine background={"bg-accent2"}>
                    ขอจอง
                </ButtonMine>
            </button>
            {/if}
            {#if currentStatus == "BI"}
            <button on:click={()=>{confirmPoup = true;done = true}}>
                <ButtonMine background={"bg-success"}>
                    สำเร็จ
                </ButtonMine>
            </button>
            {/if}
        ...
        <div class="flex flex-col items-start">
            <span>การติดต่อ : สามารถเข้ามาหน้าทริปเพื่อติดต่อได้โดยตรง</span>
            <div class="my-2"></div>
            <span class="text-xs">หมายเหตุ : หากต้องการยกเลิกกาจจอง เมื่อมีการจองสำเร็จ โปรดติดต่อกับที่พักด้วยตรง</span>
        </div>
    </div>
    ...
    <div class="flex items-center flex-col">
        <NotYetLogin/>
            <button on:click={()=>{showBookPopup = false}}>
                <ButtonMine>
                    ปิด
                </ButtonMine>
            </button>
