...
<div class="relative h-6">
    <!-- Ellipsis Button -->
    <button class="flex items-center justify-center w-12 h-6 rounded-full border-2 border-black cursor-pointer px-2 mt-[-12px]" on:click={togglePopup}>
    ...
    {#if showEditTripPopup}
      <button on:blur={() => { showEditTripPopup = false }} bind:this={editTripButton}
        on:click={()=>{editMode= !editMode; showEditTripPopup = false}}
