...
<Popup bind:isOpen={showEditPopup} hideCloseBtn={true} background={"secondary4"}>
    <div class="bg-blue-100 p-6 rounded-xl w-full shadow-lg flex flex-col items-center gap-4">
        <div class={`flex items-center bg-blue-200 text-gray-800 font-bold py-2 px-4 rounded-lg ${selectInputClass}`}>
            <Icon icon={checkpointTypeIcon.get(typeCKEdit) ?? "carbon:location-current"} class="text-xl" />
            <!-- Wrap the select in a div for custom styling -->
            <div class={`w-full ml-2 rounded-md`}>
                <select class="w-full bg-transparent text-gray-800 focus:outline-none text-base	"
        ...
        <div class="flex flex-col w-full">
            ...
            <span class="text-xs text-gray-500">หมายเหตุ : สถานที่นี้จะนำเข้าไปค้นใน google map และแสดงให้เห็นอันดับแรก</span>
        </div>
        ...
        <div class="flex flex-col w-full items-center font-bold">
            ...
            <textarea
                ...
                <div class="flex justify-center gap-4 w-full mt-4">
                    {#if !newCK}
                    <button class="bg-error text-white px-4 py-2 rounded-md  transition-all"
                    on:click={() => { removePopup = true }}>
                    ลบ
                    </button>
                    {/if}
                    <button class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-all" on:click={() => { showEditPopup = false}}>
                        ปิด
                    </button>

                    <button class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-all"
