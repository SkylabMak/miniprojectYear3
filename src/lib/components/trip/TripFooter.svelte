...
$: amountInputClass = amount !== amountChanged ? "border-2 border-warning" : "border-2 border-black"; // Update border when select is changed
</script>
...
{:else}
    <div class="flex flex-col items-center justify-center gap-4">
        ...
        <button on:click={()=>{settingTripPopup = true}}>
            <ButtonMine>
                ตั้งค่าทิป
            </ButtonMine>
        </button>
    ...
    <div class="bg-secondary4 w-fit rounded-xl">
        ...
        <div class="flex justify-between items-center mb-3">
            ...
            <div class={`flex gap-2 ${publicInputClass} rounded-full`}>
                <label class="flex items-center">
                    <input type="radio" name="public" value={true} bind:group={publicChoice} class="hidden" />
                    <span class={`px-2 py-1 text-center w-12 rounded-full cursor-pointer ${publicChoice === true ? 'bg-success border-2 border-green-700' : 'bg-gray-300'}`}>ใช่</span>
                </label>
                <label class="flex items-center">
                    <input type="radio" name="public" value={false} bind:group={publicChoice} class="hidden" />
                    <span class={`px-2 py-1 text-center w-12 rounded-full cursor-pointer ${publicChoice === false ? 'bg-error border-2 border-red-800' : 'bg-gray-300'}`}>ไม่</span>
                </label>
            </div>
        </div>
        {/if}
        ...
        <div class="flex justify-between items-center mb-3">
            ...
            <div class={`flex gap-2 ${bookingInputClass} rounded-full`}>
                <label class="flex items-center">
                    <input type="radio" name="booking" value={true} bind:group={bookingChoice}  class="hidden" />
                    <span class={`px-2 py-1 text-center w-12 rounded-full cursor-pointer ${bookingChoice == true ? 'bg-success border-2 border-green-700' : 'bg-gray-300'}`}>ใช่</span>
                </label>
                <label class="flex items-center">
                    <input type="radio" name="booking" value={false} bind:group={bookingChoice}  class="hidden" />
                    <span class={`px-2 py-1 text-center w-12 rounded-full cursor-pointer ${bookingChoice == false ? 'bg-error border-2 border-red-800' : 'bg-gray-300'}`}>ไม่</span>
                </label>
            </div>
        </div>
        {/if}
        ...
        <div class="flex justify-center mt-5 gap-4">
            <button on:click={()=>{removePopup = true}} class="bg-error text-white px-4 py-2 rounded-md hover:bg-gray-900 transition-all">ลบ</button>
            <button on:click={closePopup} class="bg-accent2 text-white px-4 py-2 rounded-md hover:bg-gray-900 transition-all">ปิด</button>
            <button on:click={saveChanges} class="bg-accent2 text-white px-4 py-2 rounded-md hover:bg-gray-900 transition-all">บันทึก</button>
        </div>
    </div>
</Popup>
<Popup bind:isOpen={removePopup} hideCloseBtn={true}>
    ...
    <div class="flex items-center gap-4 justify-center mt-4">
        <button on:click={()=>{removePopup = false}}>
            <ButtonMine>
                ปิด
            </ButtonMine>
        </button>
